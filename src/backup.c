...
backup_t *backup_open(const char *backupdir, const char *uuid) {
	...
	char *backup_path = (char *) malloc(
			strlen(backupdir) + 1 + strlen(uuid) + 1 + 4);
	strcpy(backup_path, backupdir);
	strcat(backup_path, "/");
	strcat(backup_path, uuid);

	char *mbdb_path = (char *) malloc(
			strlen(backup_path) + 1 + strlen("Manifest.mbdb") + 1 + 4);
...
char *backup_get_file_path(backup_t * backup, backup_file_t * bfile) {
	...
	char *bfntmp = (char *) malloc(
			bfile->mbdb_record->domain_size + 1 + bfile->mbdb_record->path_size
					+ 1 + 4);
...
int backup_update_file(backup_t * backup, backup_file_t * bfile) {
	...
	// find record
	int idx = backup_get_file_index(backup, bfile->mbdb_record->domain,
	...
	if (idx < 0) {
		...
		backup_file_t *oldfile = backup_file_create_from_record(
				backup->mbdb->records[idx]);
...
int backup_write_mbdb(backup_t * backup) {
	...
	char *mbdb_path = (char *) malloc(
			strlen(backup->path) + 1 + strlen("Manifest.mbdb") + 1);
...
int backup_add_file_from_data(backup_t * backup, char *domain, char *data,
		unsigned int size, char *path, int mode, int uid, int gid, int flag) {
	int ret = -1;
	backup_file_t *file = backup_file_create_with_data(data, size, 0);
